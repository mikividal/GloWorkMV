...
def create
  ...
  @comment.user = current_user # Ensure comment has a user
    ...
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: [
          ...
          turbo_stream.replace("comment_form_<%= @suggestion.id %>", partial: "suggestions/comment_form", locals: { suggestion: @suggestion }) # Ensures the form resets
        ]
      end
      format.html { redirect_to @suggestion }
    end
  else
    Rails.logger.error("COMMENT SAVE FAILED: #{@comment.errors.full_messages}") # Debugging log
    respond_to do |format|
